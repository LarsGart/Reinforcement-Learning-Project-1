...
# random 4x4 block of walls inside the grid
walls = [(random.randint(0, x-1), random.randint(0, y-1)) for i in range(x * y // 4)]
...
def touchRewardHistory(val):
    rewardHistory.append(val)
    global stabilized
    if len(set(rewardHistory[-10:])) == 1 and len(rewardHistory) > 10:
        print(rewardHistory)
        stabilized = True
    else:
        stabilized = False
...
# change any cell color to black on mouse click
def change_color(event):
    ...
    if (x, y) not in walls:
        walls.append((x, y))
        board.create_rectangle(x*Width, y*Width, (x+1)*Width, (y+1)*Width, fill="black", width=1)
